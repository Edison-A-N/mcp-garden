...
def _register_routes(public_router: APIRouter, protected_router: APIRouter) -> None:
    ...
    async def root():
        ...
        return BaseResponse(
            message="Shopping Cart API - FastAPI MCP Server", timestamp=datetime.now()
        )
    ...
    async def list_products(
        ...
        if category:
            filtered_products = [
                p for p in filtered_products if p.get("category") == category.value
            ]
        ...
        if is_available is not None:
            filtered_products = [
                p for p in filtered_products if p.get("is_available") == is_available
            ]
        start_idx = (page - 1) * size
        end_idx = start_idx + size
    ...
    @protected_router.post(
        "/cart/{user_id}/items/", response_model=CartItemResponse, status_code=201
    )
